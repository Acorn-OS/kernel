...
const BLOCK_SIZE: usize = paging::KERNEL_PG_SIZE;
...
impl Heap {
    unsafe fn init(&mut self) {
        assert!(ksyms::free_mem_adr() & paging::KERNEL_PG_MASK == 0);
        self.block_count = 4096 * 4;
        let block_desc_alloc = self.block_count;
        let block_alloc = self.block_count * BLOCK_SIZE;
        assert!(block_alloc + block_desc_alloc < ksyms::free_mem_len());
        self.block_desc_offset = ksyms::free_mem_adr();
        self.block_offset = ksyms::free_mem_adr() + self.block_count;
